"""Lifted from https://gist.github.com/alexwlchan/73933442112f5ae431cc
Author: Alex Chan
"""
    ...
    * If in a different year, print the date with the whole year
    ...
    if isinstance(past_time, str):
        ...
        try:
            ...
            raise ValueError(
                "User supplied string %s is not in ISO 8601 " "format." % past_time
            )
    elif isinstance(past_time, datetime):
        pass
    else:
        raise ValueError(
            "User-supplied value %s is neither a datetime object "
    ...
    # --------------------------------------------------------------------------
        ...
        # the date
            ...
            incl_year = past_time.year != now_time.year
            time_str = print_date(
                past_time, incl_year=incl_year, short_months=short_months
            )
